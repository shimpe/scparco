/*
[general]
title = "ScpCombinatorTester"
summary = "unit tests using some ScpParserFactory methods"
categories = "Parsing Tools"
related = "Classes/ScpParserFactory"
description = '''unit tests using some ScpParserFactory methods'''
*/
ScpCombinatorTester : UnitTest {
	*new {
		^super.new.init();
	}
	...
	test_between {
		...
		var result;
		fullText = "(hello)";
		betweenBrackets = ScpParserFactory.makeBetween(ScpStrParser("("), ScpStrParser(")"));
		result = betweenBrackets.(ScpRegexParser("[a-z]+")).run(fullText);
	...
	test_chain {
		...
		var stringParser = ScpRegexParser("[A-Za-z]+").map({ |result| (\typename: 'string', \value : result); });
		var numberParser = ScpRegexParser("[0-9]+").map({ |result| (\typename: 'number',  \value : result.asInteger); });
		var numParserResult = numberParser.run("42");
		var dicerollParser = ScpSequenceOf([
			ScpRegexParser("[0-9]+"),
			ScpStrParser("d"),
			ScpRegexParser("[0-9]+")
		]).map({ | result | (\typename : 'diceroll', \value : [ result[0].asInteger, result[2].asInteger ]) });

		var typevalueparser = ScpSequenceOf([
	...
	test_sepby {
		var p1 = ScpParserFactory.makeSepBy(ScpStrParser(",")).(ScpRegexParser("[A-Za-z]+"));
		var p2 = ScpParserFactory.makeSepByOne(ScpStrParser(",")).(ScpRegexParser("[A-Za-z]+"));
		var result;
	...
	test_parsearray {
		...
		var flat_arrayparser = betweenSquareBrackets.(commaSeparated.(ScpRegexParser("[0-9]+").map({|x| x.asInteger })));
